...
X, Y = datasets.make_blobs(n_samples=n_samples, n_features=dim, centers=2, cluster_std=[4.0, 4.0], random_state=42)
'''
...
X_train, X_test, Y_train, Y_test = train_test_split(X, Y_train, test_size=0.98, random_state=52)

n_features_list = np.arange(2, dim , 2)
distances1 = []
distances2= []
...
#high dim for verify task one
    ...
    for n_features in n_features_list:
        ...
        w_s = SVM(X_train, Y_train)
        # print(w_log)
        Y_valid = np.sign(X_train @ w_s)
        #print(Y_pre)
        #print(Y_train)
        trainrisk = zero_one_loss(Y_valid, Y_train)
